...
**ait。** 每次執行落在隔離 Git worktree。Root checkout 永遠不動。
...
**ait。** 每筆 attempt 把 intent、prompt、退出狀態、變更檔案、捕捉
...
**ait。** 失敗 attempt 留在自己的 worktree 裡審或 `discard`。主分支
從頭到尾乾淨。
...
**ait。** Repo-local memory 把過去 attempts、commits、agent memory 檔
...
**ait。** 每個 attempt 自帶 worktree。可平行跑 N 個 agent，比 attempts
...
**ait。** Promotion 是顯式動詞：`ait attempt promote <id> --to main`。
...
**ait。** Memory layer 自動匯入 `CLAUDE.md`、`AGENTS.md` 與過去 attempts，
...
**ait。** 一切活在 `.git/` 旁的 `.ait/` 裡。沒 daemon、沒 telemetry、
...
**ait。** Verifier 依實際退出狀態、檔案變更、commit 結果決定
...
**ait。** Attempts、intents、commits 用結構化 DSL 可查。可依 intent 文字、
