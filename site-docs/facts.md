...
**A:** ait is alpha. The current package version is `1.7.2` and is intended for
...
<script type="application/ld+json">
{
  ...
  "mainEntity": [
    ...
    {"@type":"Question","name":"Is ait stable?","acceptedAnswer":{"@type":"Answer","text":"ait is alpha. The current package version is 1.7.2 and is intended for local dogfooding, power users, and infra-minded engineers comfortable with Git workflows. Metadata is local to one repository under .ait/ and is not synchronized across machines; metadata export/import currently provides dry-run local plans only. Public API and CLI surface are stabilizing but not frozen."}},
