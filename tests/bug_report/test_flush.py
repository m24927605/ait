...
from ait.bug_report.seen_store import load_seen, record_seen, record_submitted
...
class DecidePromptTests(unittest.TestCase):
    ...
    def test_silent_branch_updates_count(self):
        c = collector_mod.collector()
        c.record(category="x", exc=_make_exc(), context=None, now=self._now())
        fp = c.entries()[0].fingerprint
        record_seen(fp, category="x", now=self._now(-1))  # seen 1 day ago
        decide_prompt(now=self._now())                     # should be silent
        store = load_seen()
        self.assertEqual(store[fp].count, 2)
        self.assertEqual(store[fp].last_seen_at, self._now())
